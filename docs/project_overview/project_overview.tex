...
\label{eq:game}
\end{equation}
The game is constant-sum. The historical implementation uses a Gaussian
influence kernel,
\begin{equation}
...
The kernel-agnostic implementation additionally supplies a smoothed multivariate
hyperbolic kernel, a mode-parameterized Dirichlet kernel, and independent
mode-parameterized beta factors. All are log-concave and expose
$\nabla_x\log f$ and $\nabla_x^2\log f$ to the same game dynamics. Dirichlet routing
uses the native probability simplex; the kernel comparison maps every treatment to
that same seven-dimensional domain rather than comparing different geometries.
...
embeddings into $[0,1]^L$. A configured softmax coordinate map instead produces a
native probability-simplex representation. Corpus density becomes the empirical
...
The \texttt{inflgame} package implements Equation~\ref{eq:game}: configurable
influence kernels, allocation $G$, expected utility, exact score-function utility
gradients, and kernel-matched first-bifurcation thresholds. The hyperbolic
threshold uses its generalized-eigenvalue closed form; Dirichlet and product--beta
thresholds use numerical equilibrium and root solvers. Theory-only runs optimize
positions by gradient ascent. Closed-loop runs dispatch real prompts to actual adapters.
...
Offline theory initialization and stability matching use the full calibrated KDE
resource measure. The online game-gradient update intentionally does not assume that
global knowledge: it estimates the same kernel-specific game gradient from the current
observed training batch with uniform empirical mass. An offline-KDE update remains
available only as an oracle diagnostic. Soft top-$k$ gates SFT assignment, not the
position gradient.
