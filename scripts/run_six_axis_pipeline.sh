#!/usr/bin/env bash
# Six-axis pipeline: theory init, slice figure, conditional r24 experiment.
set -euo pipefail
cd "$(dirname "$0")/.."
export PYTHONPATH=src
PY="${PY:-.venv/bin/python}"
LOG="${LOG:-results/six_axis_pipeline.log}"

exec > >(tee -a "$LOG") 2>&1
echo "=== six_axis pipeline started $(date -Is) ==="
...
pos_path = Path("results/six_axis_theory_n12/positions.json")
...
"${PY}" scripts/plot_benchmark_space_heatmaps.py \
  ...
  --title "Six-axis trait space (70/10/20 slices)"
...
bash scripts/run_six_axis_split_r24.sh

echo "=== six_axis pipeline finished $(date -Is) ==="
