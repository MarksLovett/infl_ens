...
| `modula_res.py` | `DomainBatch`, `SourceRouterBlocks`, `round_batch_indices`, `label_domain_batches` (per-benchmark rows of each round batch), `history_domain_batches` (logged pair batches + weights), `source_router_blocks`, `dominant_axis_by_merge_group` (re-keys the theory init's `pair_<a>_<b>` dominant-axis entries by merge-group name), `pair_to_benchmark_aliases`, `resolve_universal_adapter_dir`, `train_modula_res` |
...
| `adapters.py` | `AdapterRef`, `discover_adapters`, `resolve_adapter_dir`, `load_base_causal_lm` (+ `universal_adapter_dir`, merged once in fp32), `merge_universal_adapter`, `load_adapter_model` |
...
- `infl_ens.training`: `RouterTrainingConfig`, `train_router_positions`, `DomainBatch`, `SourceRouterBlocks`, `label_domain_batches`, `history_domain_batches`, `source_router_blocks`, `dominant_axis_by_merge_group`, `pair_to_benchmark_aliases`, `resolve_universal_adapter_dir`, `train_modula_res` (eager); `SFTTrainingConfig`, `sft_train_agent`, `make_chat_formatter` (lazy)
- `infl_ens.evaluation`: `AdapterEvalConfig`, `BenchmarkEvalResult`, `EvalJobConfig`, `evaluate_adapter_on_split(s)`, `evaluate_run_adapters`, `run_eval_job`, `run_unified_eval`, `universal_adapter_dir_from_config`, `final_round_from_history`, `write_eval_report`, `FittedRouter`, `fit_argmin_router`, `AdapterRef`, `discover_adapters`, `is_adapter_dir`, `resolve_adapter_dir`, `BENCHMARK_KINDS`, `load_benchmark_splits`, `subsample_split`; lazy `build_chat_formatter`, `format_chat_example`, `mean_token_nll`, `split_to_texts`
